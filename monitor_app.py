...
APP_INFO_LIST = [
    [["172.x.x.x"], "root", "xxxxx", "ReadCrawlerKafka"]
]
...
def restart(ip, password):
    try:
        ...
        ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
        ssh.connect(hostname=ip, port=22, username='root', password=password, timeout=60)
        stdin, stdout, stderr = ssh.exec_command("source /etc/profile;cd /opt/python_doc2vec_kafka;sh start.sh",
                                                 get_pty=True)
        channel = stdout.channel
        status = channel.recv_exit_status()
        ssh.close()
    ...
    content = {
        ...
        "markdown": {
            ...
            "text": "## %s\n" % time.strftime("%Y-%m-%d %X") +
                    "> ### 状态: <font color=\"#660000\">进程异常</font><br /> \n\n" +
                    "> ### 主机: {host}\n\n".format(host=host) +
                    "> ### 进程: {info}\n\n".format(info=info) +
                    "> ### 正在尝试重启进程!!!"
        },
...
def recover(host, info):
    ...
    content = {
        ...
        "markdown": {
            "title": "已恢复",
            "text": "## %s\n" % time.strftime("%Y-%m-%d %X") +
                    "> ### 状态: <font color=\"#00dd00\"></font>已恢复<br /> \n\n" +
                    "> ### 主机: {host}\n\n".format(host=host) +
                    "> ### 进程: {info}\n\n".format(info=info) +
                    "> ### 重启成功!!!"
...
def restartSuccess(host, info):
    ...
    content = {
        ...
        "markdown": {
            "title": "重启成功",
            "text": "## %s\n" % time.strftime("%Y-%m-%d %X") +
                    "> ### 状态: <font color=\"#00dd00\"></font>重启成功<br /> \n\n" +
                    "> ### 主机: {host}\n\n".format(host=host) +
                    "> ### 进程: {info}\n\n".format(info=info)
        },
        "at": {
            "atMobiles": [
                "136xxxxxxxxx", "13xxxxxxxxxx"
            ],
            "isAtAll": True
        }
    }
...
def restartFaild(host, info):
    ...
    content = {
        ...
        "markdown": {
            "title": "重启失败",
            "text": "## %s\n" % time.strftime("%Y-%m-%d %X") +
                    "> ### 状态: <font color=\"#00dd00\"></font>重启失败<br /> \n\n" +
                    "> ### 主机: {host}\n\n".format(host=host) +
                    "> ### 进程: {info}\n\n".format(info=info)
        },
        "at": {
...
def status():
    ...
    for i in range(len(APP_INFO_LIST)):
        for j in APP_INFO_LIST[i][0]:
            ...
            if codes != 0:
                result.append({'HOST': j, '进程': project})
...
if __name__ == '__main__':
    ...
    if monitor_status:
        for i in range(len(monitor_status)):
            ...
            spiderProcess = monitor_status[i]['进程']
            alarm(host, spiderProcess)
            ssh = paramiko.SSHClient()
            ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
            ssh.connect(hostname="172.x.x.x", port=22, username='root', password="xxxxx", timeout=60)
